      ...
      <section className="px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-center flex-wrap gap-2 bg-gray-100 p-1 rounded-lg mb-12 max-w-2xl mx-auto">
            {[
              { key: 'artworks', label: 'Todas as Obras', icon: Palette },
              { key: 'current', label: 'Em Cartaz', icon: Calendar },
              { key: 'upcoming', label: 'Em Breve', icon: Calendar },
              { key: 'past', label: 'Passadas', icon: Calendar }
            ].map((tab) => (
              <button
                key={tab.key}
                onClick={() => setActiveTab(tab.key as any)}
                className={`flex items-center gap-2 py-2 px-4 rounded-md font-inter font-medium text-sm transition-all duration-300 ${
                  activeTab === tab.key
                      ...
                      <div className="relative overflow-hidden bg-soft-beige rounded-2xl aspect-[4/5] shadow-lg hover:shadow-xl transition-all duration-300">
                        <img
                          src={`https://images.unsplash.com/photo-${
                            index % 4 === 0 ? '1578662996442-48f60103fc96' :
                            index % 4 === 1 ? '1594736797933-d0d6a5d80b62' :
                            index % 4 === 2 ? '1578321272176-b7bbc0679853' :
